...
describe('actionType', () => {
  test('exposes the action as `rfci` on its Id and Name', () => {
    expect(actionType.id).toEqual('.rfci');
    expect(actionType.name).toEqual('RFCI');
  });
});
...
describe('config validation', () => {
  ...
  const errorCases: Array<{ name: string; url: string; errorMsg: string }> = [
    {
      ...
      url: 'iamnotavalidurl',
      errorMsg: `"error validating action type config: error configuring send to RFCI action: unable to parse url: TypeError: Invalid URL: iamnotavalidurl"`,
    },
    {
      name: 'incomplete URL leads to error',
      url: 'example.com/do-something',
      errorMsg: `"error validating action type config: error configuring send to RFCI action: unable to parse url: TypeError: Invalid URL: example.com/do-something"`,
    },
    {
      name: 'fails when URL is not a RFCI webhook endpoint',
      url: 'http://mylisteningserver:9200/endpoint',
      errorMsg: `"error validating action type config: error configuring send to RFCI action: url must begin with https://api.recordedfuture.com/collective-insights/detections"`,
    },
  ];
  ...
  test("config validation returns an error if the specified URL isn't added to allowedHosts", () => {
    ...
    expect(() => {
      ...
      `"error validating action type config: error configuring send to RFCI action: target url is not present in allowedHosts"`
    );
  });
});
      ...
      Object {
        ...
        "logger": Object {
          ...
          "debug": [MockFunction] {
            "calls": Array [
              Array [
                "response from RFCI action \\"some-id\\": [HTTP 200] ",
              ],
            ],
            "results": Array [
              Object {
